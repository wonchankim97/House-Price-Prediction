...
class Imputator(TransformerMixin):
    ...
    def transform(self, df):
        ...
        df['Condition2'] = df.apply(lambda x:1 if x['Condition2']=='PosN' else 0, axis=1).astype(str)
...
class Standarizer(TransformerMixin):
    ...
    def transform(self, df):
        columns = [n for n in df.columns if (df[n].dtype == int) | (df[n].dtype == float)]
        skewed = [columns[i] for i,v in enumerate(stats.skew(df.loc[:,columns])) if v>0.6]
        for i in skewed:
            if min(df[i])==0:
                df[i] = df[i]+0.00001
